...
if ($cntid == 1) {
    ...
    function normalizeTextPHP($text){
        ...
        // 3. 【最優先】特例対応
        if (mb_strpos($text, '興野町住宅') !== false) {
            return '興野町';
        }
        if (mb_strpos($text, '春江町住宅') !== false) {
            return '春江町'; // 「町」を残して「住宅」を消す特例
        }

        // 4. 末尾が数字・記号で終わる場合のみ、その部分を削除
...
<head>
    ...
    .task-item-link { 
        ...
        align-items: center; width: 100%; padding: 15px; box-sizing: border-box; 
    }
    .normalize-test { cursor: pointer; }
    .normalize-test:hover { background-color: #e9e9e9; }
...
<?php require '../require/header.php'; ?>
    ...
    <div id="contener">
        ...
        <ul class="project-list">
            <?php foreach ($cached_tasks as $row): ?>
                ...
                <li>
                    <?php if ($t_db): ?>
                        <div class="task-item-link normalize-test"
                             data-content="<?= htmlspecialchars($content, ENT_QUOTES, 'UTF-8') ?>"
                             data-code="<?= htmlspecialchars($t_db['code'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                             data-syubetu="<?= htmlspecialchars($t_db['syubetu'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                             data-name="<?= htmlspecialchars($t_db['name'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                             data-address="<?= htmlspecialchars($t_db['address'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                             data-map="<?= htmlspecialchars($t_db['map'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                    <?php else: ?>
                        <div class="task-item-link" style="cursor: default;">
                    <?php endif; ?>
                        
                        <div class="task-info">
                            <span class="task-content" style="<?= !$t_db ? 'color: #888 !important;' : '' ?>">
                                <?= htmlspecialchars($content); ?>
                            </span>
                            
                            <div class="task-meta" style="display: flex; flex-wrap: wrap; align-items: center; gap: 10px;">
                                <?php if (!empty($due_date)): ?>
                                    <span class="meta-date" style="white-space: nowrap;">
                                        <?= todoistDateLabel($due_date); ?>
                                    </span>
                                <?php endif; ?>
                                
                                <?php if (!empty($t_db['address'])): ?>
    ...
    function numToKanji(num){
        ...
        if(num === 10) return '十';
        if(num < 20) return '十' + K[num % 10];
        if(num % 10 === 0) return K[Math.floor(num / 10)] + '十';
        return K[Math.floor(num / 10)] + '十' + K[num % 10];
    }
    ...
    function normalizeText(text){
        ...
        // 局名を先に消す
        text = text.replace(/(交通局|水道局|総務局|教育庁|消防宿舎)/g,'');
        text = text.trim();

        let target = text.split(/[\s　]/)[0];

        // 特例対応
        if (target.includes('興野町住宅')) return '興野町';
        if (target.includes('春江町住宅')) return '春江町';

        // 末尾が数字・記号で終わる場合のみ、その部分をカット
        if (target.match(/[0-9０-９\-\/ーの番号棟室]$/u)) {
            const match = target.match(/^(.*[^\x00-\x7f])([0-9０-９\-\/ーの番号棟室]+)$/u);
            if (match) {
                target = match[1];
            }
        }

        target = target.replace(/第([0-9０-９]+)住宅/u, (_, n)=> '第' + numToKanji(toHalfWidth(n)));
        target = target.replace(/第([一二三四五六七八九十〇]+)住宅/u,'第$1');
        
        if (target.match(/(.+?)町住宅/)) {
            target = target.replace(/(.+?)町住宅/, '$1住宅');
        } else if (target.includes('寮')) {
            // 寮の場合はそのまま
        } else {
            target = target.replace(/住宅/g, '');
        }
        
        target = target.replace(/([0-9]+)丁目/g, (_, n)=> numToKanji(n)+'丁目');
        return target;
    }

    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('.normalize-test').forEach(div => {
            div.addEventListener('click', e => {
                ...
                params.append('code', div.getAttribute('data-code') || '');
                params.append('syubetu', div.getAttribute('data-syubetu') || '');
                params.append('address', div.getAttribute('data-address') || '');
                params.append('map', div.getAttribute('data-map') || '');
                location.href = '../building.php?' + params.toString();
            });
        });
    });
    </script>
